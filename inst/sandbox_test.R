...
# 
# past_forecasts <- crowdforecastr:::load_past_forecasts("external_ressources/processed-forecast-data/")
# data.table::fwrite(x = past_forecasts, file = "external_ressources/past_forecasts.csv")
...
past_forecasts <- data.table::fread("external_ressources/past_forecasts.csv") %>%
  dplyr::mutate(target_end_date = as.Date(target_end_date)) %>%
  dplyr::mutate(target_type = ifelse(target_type == "case", "cases", "deaths"))

past_forecasts <- dplyr::full_join(past_forecasts, obs_filt %>%
